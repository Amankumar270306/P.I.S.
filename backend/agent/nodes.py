    ...
    try:
        ...
        if action == "create" and title:
            ...
            if energy_log:
                ...
                pct = int((energy_log.used_capacity / energy_log.total_capacity) * 100) if energy_log.total_capacity > 0 else 0
                bar_filled = pct // 10
                bar_empty = 10 - bar_filled
                bar = "🟩" * bar_filled + "⬜" * bar_empty
                result = (
                    f"⚡ **Energy Status** ({today.strftime('%b %d')})\n"
                    f"{bar} {pct}% used\n"
                    f"• Used: **{energy_log.used_capacity:.1f}** / {energy_log.total_capacity:.1f} points\n"
                    f"• Remaining: **{remaining:.1f}** points ({remaining * 10:.0f} minutes)"
                )
            else:
                result = f"⚡ **Energy Status** ({today.strftime('%b %d')})\nFull capacity! **90.0** points available (900 minutes)"
            ...
            count = db.query(models.Task).filter(
                models.Task.user_id == get_active_user_id()
            ).count()
            if count > 0:
...
def document_node(state: AgentState, db: Session) -> dict:
    ...
    messages = [
        {"role": "system", "content": prompt},
        {"role": "user", "content": f"Analyze this document: {doc_title}\n\nUser request: {user_input}"},
    ]
